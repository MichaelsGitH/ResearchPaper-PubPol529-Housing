...
# Population under study is renters, excludes all non-renters. obs = 8461
data <- subset(data, data$RENT > 0)

# Change race data from factor to integer
data$HHSEX_NEW <- as.integer(data$HHSEX)
data$HHRACE_NEW <- as.integer(data$HHRACE)
data$HHMAR_NEW <-as.integer(data$HHMAR)

# Exclude all obs with not applicable var for Sex and Race variable
...
#### At this point, observations in data_new are renters that have
#### been asked the HHSEX, HHRACE, HHGRAD question (applicable), n = 7174
...
# Exclude obs that pay more than monthly income in housing costs
data_new1 <- subset(data_new1, data_new1$CB < 1)

# Exclude extreme obs, less than 100$ in monthly rent
data_new1 <- subset(data_new1, data_new1$RENT >= 100)

# Create rent burden indicator, 1 if equal or more than 30% of monthly income to rent, 0 else
...
ggplot(data_use, aes(x=factor(data_use$HHRACEALL), y=data_use$RENT)) + stat_summary(fun.y="mean", geom="bar")
...
###########################################################
### T-test for covariates difference in means #############
###########################################################
